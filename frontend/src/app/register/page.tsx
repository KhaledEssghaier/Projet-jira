...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    // MOCK: inscription toujours acceptée si email et mot de passe non vides
    if (!email || !password) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded shadow w-full max-w-sm flex flex-col gap-4">
        ...
        <button type="submit" className="bg-blue-600 text-white py-2 rounded hover:bg-blue-700">S’inscrire</button>
      </form>
    </div>
  );
}
